...
let models = [teapotMesh, sphereMesh];
let phongConfig = [teapotConfig, sphereConfig];
let phongMaterial = [teapotMaterial, sphereMaterial];
...
let gouraudConfig = [teapotConfig, sphereConfig];
let gouraudMaterial = [teapotMaterial, sphereMaterial];
let currentMaterial = 0 // 0 for phong, 1 for gouraud 
...
document.addEventListener('keydown', function(event) {
    if(event.code == 'KeyS') {
        ...
        if(currentMaterial == 0) {
            models[0].material = phongMaterial[0];
            models[1].material = phongMaterial[1];
        }
        else if(currentMaterial == 1) {
            models[0].material = gouraudMaterial[0];
            models[1].material = gouraudMaterial[1];
        }
    }
});
